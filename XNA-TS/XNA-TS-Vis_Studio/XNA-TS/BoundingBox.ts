export class BoundingBox {
    ...
    public static CornerCount: number = 8;
    public Min: Vector3;
    public Max: Vector3;
    ...
    public Intersects(box: BoundingBox): boolean {
        return <number>this.Max.X >= <number>box.Min.X && <number>this.Min.X <= <number>box.Max.X && (<number>this.Max.Y >= <number>box.Min.Y && <number>this.Min.Y <= <number>box.Max.Y) && (<number>this.Max.Z >= <number>box.Min.Z && <number>this.Min.Z <= <number>box.Max.Z);
    }
    ...
    public SupportMapping(v: Vector3, result: Vector3): void {
        result.X = <number>v.X >= 0.0 ? this.Max.X : this.Min.X;
        result.Y = <number>v.Y >= 0.0 ? this.Max.Y : this.Min.Y;
        result.Z = <number>v.Z >= 0.0 ? this.Max.Z : this.Min.Z;
    }
